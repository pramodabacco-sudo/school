...
const Section = ({ icon: Icon, title, color = "blue", children }) => (
  <div className={`rounded-xl border border-gray-100 p-5 bg-${color}-50/30`}>
    <div className="flex items-center gap-2 mb-4">
      <div
        className={`w-7 h-7 rounded-lg bg-${color}-100 flex items-center justify-center`}
      >
...
export default function AddStudent({ onClose, closeModal, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const fields = {
        ...
        state: f.state,
        grade: f.grade,
        className: f.class, // backend field is "className" not "class"
      ...
      const piRes = await fetch(
        `${API_URL}/api/students/${sid}/personal-info`,
        {
          method: "POST",
          headers: authHeaders(),
          body: fd,
      ...
      if (!piRes.ok)
        throw new Error(piData.message || "Failed to save personal info");
    ...
    try {
      ...
      extraDocs.forEach((doc) => {
        if (doc.file)
          allFiles.push({
            file: doc.file,
            documentName: "CUSTOM",
            customLabel: doc.label || "Custom Document",
          });
      });
      ...
      if (allFiles.length > 0) {
        ...
        const res = await fetch(
          `${API_URL}/api/students/${sid}/documents/bulk`,
          {
            method: "POST",
            headers: authHeaders(),
            body: fd,
  ...
  // ── Doc helpers ────────────────────────────────────────────────────────────
  const setFixed = (id, file) =>
    setFixedDocFiles((p) => ({ ...p, [id]: file }));
  const removeFixed = (id) => {
    setFixedDocFiles((p) => ({ ...p, [id]: null }));
    if (fixedRefs.current[id]) fixedRefs.current[id].value = "";
  };
  const addExtra = () =>
    setExtraDocs((p) => [...p, { id: Date.now(), label: "", file: null }]);
  const removeExtra = (id) => setExtraDocs((p) => p.filter((d) => d.id !== id));
  const setExtraLbl = (id, v) =>
    setExtraDocs((p) => p.map((d) => (d.id === id ? { ...d, label: v } : d)));
  const setExtraFile = (id, file) =>
    setExtraDocs((p) => p.map((d) => (d.id === id ? { ...d, file } : d)));
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-gray-400">
                        Upload official student documents
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 bg-amber-50/60">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-gray-400">
                        Optional — medical cert, passport, caste cert, etc.
                      </p>
                    </div>
                  </div>
                  <button
                  ...
                  {extraDocs.length === 0 ? (
                    <div className="flex flex-col items-center py-8 text-center">
                      ...
                      <p className="text-xs text-gray-300 mt-1">
                        Click "Add" to upload any extra file
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {extraDocs.map((doc, idx) => (
                        ...
                        >
                          ...
                          <input
                            ...
                            value={doc.label}
                            onChange={(e) =>
                              setExtraLbl(doc.id, e.target.value)
                            }
                          ...
                          />
                          <label
                            className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold cursor-pointer transition shrink-0 ${
                              doc.file
                          ...
                          >
                            ...
                            <input
                              ...
                              accept=".pdf,.jpg,.jpeg,.png,.doc,.docx"
                              onChange={(e) =>
                                setExtraFile(doc.id, e.target.files[0])
                              }
                              className="hidden"
                            />
                          </label>
